...
flist_channels_data = dir("data",
"^channels-data_\\d{2}-\\d{4}[.]csv$", full.names=TRUE
)
channels_data = lapply(flist_channels_data, fread) |> rbindlist(use.names=TRUE)
renamer = \(x) sub("(snippet|statistics|contentDetails|topicDetails)[.]", "", x)
setnames(channels_data, renamer)
setnames(channels_info, renamer)
channels_data = merge(channels_data, channels_info,by = "title")[order(retrievedAt), ]

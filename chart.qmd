...
channels_data = merge(channels_data, channels_info, "title")[order(retrievedAt), ]
...
channels_data[, viewCountPerVideo := floor(viewCount / videoCount)
                  ...
                  ][, viewCountPerYear := viewCount/age
                    ][, title := gsub("[ -]*\\p{Ethiopic}+[ -]*", "", title, perl = TRUE)]
...
subtitle = sprintf("Data access date: <span style='color:red;'>%s</span>", retrieved_at)
...
flipped_bar_plot <-
  function(data, x, y, label, ..., geom.col.params = list(), geom.text.params = list()) {
    geom.col.params = modifyList(list(...), geom.col.params)
    geom.text.params = modifyList(list(...), geom.text.params)
    ggplot(data = data, mapping = aes({{ x }}, y = reorder({{ y }}, {{ x }}), label = {{ label }}), ...) +
      do.call("geom_col", modifyList(list(fill = "#55bdb4", alpha = .5, width = .8), geom.col.params)) +
      do.call("geom_text", modifyList(list(hjust = -0.05), geom.text.params)) +
      scale_x_continuous(
...
```{r}
flipped_bar_plot(data_latest, viewCount, title, scales::label_comma()(viewCount))
```

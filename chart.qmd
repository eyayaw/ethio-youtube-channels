...
lookup = list(
  var = c("viewCount", "subscriberCount", "videoCount", "viewCountPerVideo", "age_in_days"),
  label = c("#Views", "#Subscribers", "#Videos", "#Views per video", "Age (#days)"),
  color = c("#8298f8", "#52b439", "#f68060", "#55bdb4", "#912566"),
  xupper_lim_expand_mult = c(0.18, .1, .075, .1, .1),
  scale = c(1e-6, rep(1e-3, times = 3), 1),
  labeller = c(
    scales::label_math(.x %*% 10^6),
    replicate(4, scales::label_comma(suffix = "k"))
  )
)
...
for (.var in names(subplots)) {
  ...
  subplots[[.var]] = ldata[[.var]] |> 
    transform(
      title = reorder(title, count), scount = count * .aes$scale,
      hjust = ifelse(count == max(count), 1, 0)
      ) |> 
    ggplot(aes(scount, title)) +
